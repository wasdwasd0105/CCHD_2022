...
class MainWindow(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        # to input N- Normal , X - Not Normal 
        n_button = self.create_button('N')
        n_button.pressed.connect(self.input_btn_pressed_n)
        self.grid_layout.addWidget(n_button, 3, 2, 1, 1)

        x_button = self.create_button('X')
        x_button.pressed.connect(self.input_btn_pressed_x)
        self.grid_layout.addWidget(x_button, 3, 1, 1, 1)
    ...
    def input_btn_pressed_n(self):
        if self.locked:
            return
        patient = self.stack[0] * 100000 + self.stack[1] * 10000 + self.stack[2] * 1000 + self.stack[3] * 100 + self.stack[4] * 10 + self.stack[5]
        patient = str(patient)
        patient = 'N' + patient
        filename = os.path.join(DATA_DIR, PATIENT_DATA_FILENAME)
        ...
        if os.path.exists(filename):
            ...
            with open(filename, "w") as f:
                f.write("time,Patient_ID\n")
                f.write("{},{}\n".format(time, patient))
        self.lock()
        exit()

    def input_btn_pressed_x(self):
        if self.locked:
            return
        patient = self.stack[0] * 100000 + self.stack[1] * 10000 + self.stack[2] * 1000 + self.stack[3] * 100 + self.stack[4] * 10 + self.stack[5]
        patient = str(patient)
