...
# %% input necessary files here

curdir = os.path.dirname(os.path.realpath(__file__))
config = configparser.ConfigParser()
config.read(os.path.join(curdir, '..', 'Config', 'config.ini'))

AWAD_model = os.path.join(curdir, config['AWAD_model']['model_name'])
AWAD_features = config['AWAD_model']['model_features']
AWAD_features = AWAD_features.split(',')

CCHD_model = os.path.join(curdir, config['CCHD_model']['model_name'])
CCHD_features = config['CCHD_model']['model_features']
CCHD_features = CCHD_features.split(',')

path_global = os.path.join(curdir, '..', 'Data', 'tmp', 'Recorded')
preprocessed_path_global = os.path.join(curdir, '..', 'Data', 'tmp', 'Preprocessed')

tested_path = os.path.join(curdir, '..', 'Data', 'Tested')
extracted_features_path = os.path.join(curdir, '..', 'Data')
...
def AWAD_feature_select(incoming_features):
    feature_columes = [False,False,False,False,False,False,False,False,False,False,False,False,False,False]
...
AWAD_features_columes = AWAD_feature_select(AWAD_features)
#print(AWAD_features_columes)

# %%
def CCHD_feature_select(incoming_features):
    feature_columes = [False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False,False]
    for feature in incoming_features:
...
CCHD_features_columes = CCHD_feature_select(CCHD_features)
#print(len(CCHD_features_columes))
#print(CCHD_features_columes)

# %%
def file_into_folder(path, new_path):
...
# %%
file_into_folder(path_global, preprocessed_path_global)

#### Now let's analyze the number of cases

cases_list = sorted(os.listdir(preprocessed_path_global))
...
for case in cases_list:
    file_list = sorted(os.listdir(os.path.join(preprocessed_path_global, case)))
    if len(file_list) == 6:
...
# %%
def take_division(fp,folder_path,filename,isPleth):
    if isPleth:
...
def clean_folder(path):
    ...
    for case in cases_list:
        ...
        if len(file_list) == 6:
            ...
            isMili1 = take_division(pleth_hand_path,folder_path,str(case)+'_pleth_hand.csv',True)
            #Foot Pleth
            pleth_foot_path = os.path.join(path, case, file_list[2])
            isMili2 = take_division(pleth_foot_path,folder_path,str(case)+'_pleth_foot.csv',True)
            #Hand Pulseox
            pulseox_hand_path = os.path.join(path, case, file_list[5])
            _ = take_division(pulseox_hand_path,folder_path,str(case)+'_pulseox_hand.csv',False)
            #Pleth Pulseox
            pulseox_foot_path = os.path.join(path, case, file_list[4])
            _ = take_division(pulseox_foot_path,folder_path,str(case)+'_pulseox_foot.csv',False)
...
cases_to_exclude = clean_folder(preprocessed_path_global)

# %%
def grab_data_files(path, isPleth):
    if isPleth:
        name_search = 'pleth'
    else:
        name_search = 'pulseox'
...
def mount_df(beat_sequences):
    ...
    for key in beat_sequences:
        ...
        if df_count == 0:
            dataset_new = make_df(slice_df,'?',key)
            df_count = df_count + 1
        else:
            dataset_partial = make_df(slice_df,'?',key)
            ...
            if counter == 0:
                ...
                if (ID.split('_')[0]== ID_1.split('_')[0]) and (ID.split('_')[1]== ID_1.split('_')[1]) and (ID.split('_')[2]== ID_1.split('_')[2]):
                    if len(peaks)>0 and len(peaks_1)>0:
                        ...
                        comp = dtw.dtw(bn,bn_1)
                        avg_len = (len(bn)+len(bn_1))/2


                        dtwdist = comp.distance
...
# %%
def ml_awad_model_test(case, ForH, data_test, printFull = False, col=0, isStd = True):
    ...
    ## select features
    columns_to_bool = AWAD_features_columes
    X_test = X_test[:,columns_to_bool]   ## print the columns that were used
    model = joblib.load(AWAD_model)
    ...
    if (ForH == 'hand'):
        result_output['hand_awad_ratio'] = res
        result_output['hand_awad_time'] = awad_time
    if (ForH == 'foot'):
        result_output['foot_awad_ratio'] = res
        result_output['foot_awad_time'] = awad_time

    Xtest_og['awad_gt'] = predictions    
    return Xtest_og, columns_to_bool

# %%
def reconstruct_waveforms(beat_df,beat_sequences,awad_pred,reconstructed_beats):
    gt = []
    previous_pred = 1
...
def pleth_features(fp_hand,fp_foot,case,take,rfe_cols, reconstructed_beats):
    df_hand = change_df(fp_hand)
    df_foot = change_df(fp_foot)
    
    reconstructed_beats, rfe_cols = artifact_removed(df_hand,case,take,rfe_cols,True,reconstructed_beats)
    reconstructed_beats, rfe_cols = artifact_removed(df_foot,case,take,rfe_cols,False,reconstructed_beats)
    
    
    slope = grab_slope(reconstructed_beats)
    delay = grab_delay(reconstructed_beats)
    
    return slope, delay, rfe_cols,reconstructed_beats 

def grab_slope(reconstructed_beats):
    slope = []
...
def grab_delay(reconstructed_beats):
    pre_delay = []
    delay = []
    baseline_factor = 100
...
def artifact_removed(seq, case, take,rfe_cols,isHand,reconstructed_beats):
    beat_sequences = {}
    key_count = 0
    ...
    beat_df = mount_df(beat_sequences)
    
    awad_features = GrabInfo(beat_df)
    
    ## up to here it is looking fine both awad features and beat_df
    
    awad_pred, rfe_cols = ml_awad_model_test(case, ForH ,awad_features, printFull = False, col=rfe_cols, isStd = True)
    
    reconstructed_beats = reconstruct_waveforms(beat_df,beat_sequences,awad_pred,reconstructed_beats)

    return reconstructed_beats, rfe_cols

# %%
def extract_all_features(fp,csv_path,cases_to_exclude):
    ## need to add reconstructed beats dict here
    reconstructed_beats = {}
    paths = grab_data_files(fp, True)
    ct = 0
    case_ct = 0
    processed = False
    rfe_cols = 0
    for path in paths:
        ...
        if case_name not in cases_to_exclude:
            ...
            if ForH == 'hand':
                ...
                if os.path.exists(path) and os.path.exists(foot_path) and os.path.exists(pulseox_hand_path) and os.path.exists(pulseox_foot_path):
                    PAI_f,SPO2_f,HR_f = pulseox_features(pulseox_foot_path)
                    PAI_h,SPO2_h,HR_h = pulseox_features(pulseox_hand_path)
                    
                    slope, delay, rfe_cols, reconstructed_beats = pleth_features(path, foot_path,case_name,take_name,rfe_cols, reconstructed_beats)
        ...
        if processed:
            ...
            result_output['extract_time'] = elapsed_time
            print("Got delay and slope features for: ", case_name, "in take: ", take_name)
            print("Calculated in: ", elapsed_time)
    if not os.path.exists(os.path.join(csv_path, extracted_csv)):
        features.to_csv(os.path.join(csv_path, extracted_csv))
    else:
        features.to_csv(os.path.join(csv_path, extracted_csv), mode='a', header=False)

# %%
def ml_cchd_detection(data_path, printFull = False, col=0, isStd = True):
    ...
    columns_to_bool = CCHD_features_columes
    
    X_test = X_test[:,columns_to_bool]
 
    model = joblib.load(CCHD_model)
    ...
    for prediction in y_pred:
        if prediction:
            ...
            result_output['case_name'] = case_name
            result_output['result'] = 'CCHD'
            print("Case ", case_name, "is diagnosed as CCHD")
        else:
            case_name = eval_data.ID.tail(1).tolist()[0]
            result_output['case_name'] = case_name
            result_output['result'] = 'Healthy'
            print("Case ", case_name, "is diagnosed as healthy")
    cchd_etime = time.time()
    cchd_time = cchd_etime - cchd_stime
    result_output['cchd_running_time'] = cchd_time
    time2 = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S.%f")
    result_output['test_time'] = time2
    ##finalize function, return values of spec, sens and aucs for user to tune model accordingly
    return eval_data

def clean_procssed_folder():
    file_list = sorted(os.listdir(preprocessed_path_global))
    for case in file_list:
        if os.path.exists(os.path.join(tested_path, case)):
            shutil.rmtree(os.path.join(tested_path, case))
        shutil.move(os.path.join(preprocessed_path_global, case), os.path.join(tested_path, case))

# %%
extract_all_features(preprocessed_path_global, extracted_features_path ,cases_to_exclude)
Complete_Features = os.path.join(extracted_features_path, extracted_csv)
evaled_ds = ml_cchd_detection(Complete_Features)
print(result_output)

res_out = pd.DataFrame(result_output, index=[0])
if not os.path.exists(os.path.join(extracted_features_path, CCHD_csv)):
    res_out.to_csv(os.path.join(extracted_features_path, CCHD_csv))
else:
    res_out.to_csv(os.path.join(extracted_features_path, CCHD_csv), mode='a', header=False)

clean_procssed_folder()
